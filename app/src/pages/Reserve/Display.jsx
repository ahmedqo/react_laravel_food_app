...
import { deleteReserve, getReserve, updateReserve } from "../../actions/ReserveAction";
...
export default function () {
	...
	useEffect(() => {
		(async () => {
			setReserves(null);
			const [type, data] = await getReserve();
			if (type) {
				setReserves(data);
			}
		})();
	...
	const columns = [
		{
			field: "id",
			label: "#ID",
			render: (prop) => <Title xs={{ fontSize: SIZE.Small, width: "max-content" }}>#{prop.id}</Title>,
			xs: {
				width: 160,
			},
		},
			...
			render: (prop) => (
				...
				>
					<Button
						...
						onClick={() => {
							update(prop.id, prop.status > 0 ? -1 : 1);
						}}
					>
