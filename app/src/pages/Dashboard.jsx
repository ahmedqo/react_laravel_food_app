...
export default function () {
	...
	const termine = days
		.map((m, i) =>
			orders?.filter((od) => new Date(od.created_at).toString().slice(0, 10) === m && od.status === 3).reduce((total, od) => total + od.total, 0)
		)
		.reduce((a, e) => a + e, 0);
	const prepare = days
		.map((m, i) =>
			orders?.filter((od) => new Date(od.created_at).toString().slice(0, 10) === m && od.status === 2).reduce((total, od) => total + od.total, 0)
		)
		.reduce((a, e) => a + e, 0);
	const confirm = days
		.map((m, i) =>
			orders?.filter((od) => new Date(od.created_at).toString().slice(0, 10) === m && od.status === 1).reduce((total, od) => total + od.total, 0)
		)
		.reduce((a, e) => a + e, 0);
	const attente = days
		.map((m, i) =>
			orders?.filter((od) => new Date(od.created_at).toString().slice(0, 10) === m && od.status === 0).reduce((total, od) => total + od.total, 0)
		)
		.reduce((a, e) => a + e, 0);
	const annulee = days
		.map((m, i) =>
	...
	const Total = {
		cash: orders?.reduce((t, e) => {
			if (e.status === 3) return t + e.total;
			else return t;
		}, 0),
		pendding: orders?.reduce((t, e) => {
			if (e.status !== 3 && e.status !== -1) return t + e.total;
			else return t;
		}, 0),
		cancel: orders?.reduce((t, e) => {
